...
// If any error happens, do nothing. TODO: bubble up and send email
function challengePlayersChange (challengeId) {
  ...
  db.challenges.findOne({ _id: challengeId }, function (err, challenge) {
    ...
    db.users.find({ _id: { $in: challengersIds } }, { _id: 1, name: 1 }, function (err, challengers) {
      ...
      if (challenge.currentChallenger && challengersIds.indexOf(challenge.currentChallenger._id) === -1) {
        updateQuery.$unset = { currentChallenger: true };
      }
      if (!challenge.currentChallenger && challengers.length > 0) {
        updateQuery.$set.currentChallenger = challengers[0];
      }
