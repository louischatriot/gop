...
routes.changeNegotiableParameters = function (req, res) {
  ...
  db.challenges.findOne({ _id: req.params.id }, function (err, challenge) {
    ...
    if ((req.session.user._id === challenge.creatorId) && req.body.challengerId && (challenge.currentChallengerId !== req.body.challengerId)) {
      changed = true;
      updateQuery.$set.currentChallengerId = req.body.challengerId;
    }

    if (((req.session.user._id === challenge.creatorId) || (req.session.user._id === challenge.currentChallengerId)) && req.body.handicap && (req.body.handicap !== challenge.handicap)) {
      changed = true;
    ...
    // Send response right away and update db asynchronously if needed
