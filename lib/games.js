    ...
    db.games.update({ _id: game._id }, { $set: { moves: serializedMoves, currentMoveNumber: playedMove.n } }, {}, function () {
      ...
      if (gameEngine.isGameFinished()) {
        db.games.update({ _id: game._id }, { $set:{ status: db.games.statuses.FINISHED } }, {}, function () {
          realtime.emit('game.finished');   // Using realtime event emitter even if nothing to do with io, for consistency
        });
      }
...
routes.reviewPlayApi = function (req, res) {
  ...
  db.reviews.findOne({ _id: req.params.id }, function (err, review) {
    ...
    db.reviews.update({ _id: review._id }, { $set: { moves: serializedMoves, currentMoveNumber: playedMove.n } }, {}, function () {
      ...
      // TODO: keep a real time list of current reviews
...
realtime.on('openGame.leave', function (msg) {
  ...
  if (usersIds.length === 0) {
    db.games.findOne({ _id: msg.gameId }, function (err, game) {
      if (err || !game) { return; }
      if (game.status !== db.games.statuses.ONGOING) { return; }

      db.games.update({ _id: msg.gameId }, { $set: { status: db.games.statuses.STALE } }, {}, function () {
        updateGamesLists();

        // If game is still stale after the stale delay, finish it
        staleGamesTimeouts[msg.gameId] = setTimeout(function () {
          console.log("=====================");
          db.games.update({ _id: msg.gameId, status: db.games.statuses.STALE }, { $set: { status: db.games.statuses.FINISHED, result: db.games.results.DRAW, resultExpanded: "Both players left the game before the end" } }, {}, function () {
            updateGamesLists();
          });
        }, config.staleGameCleanupDelay);
      });
    });
  }
});
...
realtime.on('openGame.join', function (msg) {
  var usersIds = Object.keys(realtime.getGamePlayersSockets(msg.gameId));
  if (usersIds.length !== 0) {
    db.games.update({ _id: msg.gameId, status: db.games.statuses.STALE }, { $set: { status: db.games.statuses.ONGOING } }, {}, function () {
      clearTimeout(staleGamesTimeouts[msg.gameId]);
      updateGamesLists();
    });
  }
});
