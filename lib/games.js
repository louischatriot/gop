...
routes.getGameStateApi = function (req, res) {
  db.games.findOne({ _id: req.params.id }, function (err, game) {
    ...
    return res.status(200).json({ moves: game.moves });
  });
};
...
routes.playApi = function (req, res) {
  ...
  db.games.findOne({ _id: req.params.id }, function (err, game) {
    ...
    db.games.update({ _id: game._id }, { $set: { moves: serializedMoves } }, {}, function () {
      // TODO: handle err
      ...
      Object.keys(socketsByPlayer).forEach(function (userId) {
        socketsByPlayer[userId].forEach(function (socket) {
          socket.emit('game.movePlayed', { playedMove: playedMove.getOwnData() });
        });
      });
