...
routes.pastGames = function (req, res) {
  db.games.find({ status: db.games.statuses.FINISHED }).sort({ createdAt: -1 }).limit(config.infiniteListBatchSize).exec(function (err, pastGames) {
...
routes.pastGamesApi = function (req, res) {
  ...
  db.games.find({ status: db.games.statuses.FINISHED, createdAt: { $lt: latestGameTime } }).sort({ createdAt: -1 }).limit(config.infiniteListBatchSize).exec(function (err, pastGames) {
