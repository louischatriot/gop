...
routes.playApi = function (req, res) {
  ...
  console.log(req.body.previousMoveN);
  console.log(move);

  return res.status(598).json({ yo: true });
  ...
  db.games.findOne({ _id: req.params.id }, function (err, game) {
    ...
    gameEngine.replaceGameTree(Move.deserialize(game.moves));
    ...
    db.games.update({ _id: game._id }, { $set: { moves: gameEngine.movesRoot.serialize() } }, {}, function () {
      // TODO: handle err
